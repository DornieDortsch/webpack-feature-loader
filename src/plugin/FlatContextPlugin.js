...
class FlatContextPlugin {
    constructor(request, context, extRegExp) {
        ...
        this.addFlattenContextMapResolver = DependenciesResolver(extRegExp, FlatContextMap(extRegExp))
    }

    apply(compiler) {
        compiler.hooks.contextModuleFactory.tap("FlatContextPlugin", cmf => {
            cmf.hooks.beforeResolve.tap("FlatContextPlugin", this.replaceContext)
            cmf.hooks.afterResolve.tap("FlatContextPlugin", this.addFlattenContextMapResolver)
        })
    }
}
